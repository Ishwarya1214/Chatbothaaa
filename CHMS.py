...
db = mysql.connector.connect(
    ...
    password="Ishwarya12@",  # your MySQL password
...
def register_user(full_name, dob, username, password):
...
def login_user(username, password):
    ...
    with open("users.txt", "r") as f:
        users = f.readlines()
        for user in users:
            stored_name, stored_dob, stored_username, stored_password = user.strip().split(",")
...
def schedule_appointment(patient_name, doctor_id, appointment_date):
    try:
        ...
        db.commit()
        st.success(f"Appointment successfully scheduled with Doctor ID {doctor_id} on {appointment_date}.")
    except Exception as e:
        st.error(f"Error: {str(e)}")

# Streamlit interface
def main():
    st.title("Hospital Appointment Management System (HAMS) Chatbot")
    page_element="""
    <style>
    [data-testid="stAppViewContainer"]{
       background-image: url("https://th.bing.com/th/id/OIP.nAGy-6kph0x_9XpRBcQPyAHaEe?rs=1&pid=ImgDetMain");
    ...
    menu = ["Home", "Register", "Login", "Schedule Appointment", "View Doctors"]
